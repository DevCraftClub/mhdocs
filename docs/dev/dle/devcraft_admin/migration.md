...
| `MH_*` константы | `DEVCRAFT_*` (см. [Paths](reference/back-end/classes/Paths.md)) |
...
Путь к локалям можно проверить в [Настройка](guides/manage.md) → **Путь до файлов языков**.
...
Таблица логов в БД: `devcraft_logs` (см. [LogRecord](reference/back-end/classes/LogRecord.md)). Старые записи `{prefix}_maharder_*` **не импортируются** автоматически.

### 6. Сторонние модули автора

Каждый legacy-модуль под `engine/inc/maharder/_modules/` требует **переписывания** под структуру `devcraft/src/modules/{Name}/` (manifest, Pages, Ajax). Используйте [генератор модулей](guides/new_module.md) как стартовую точку.
