...
Для быстрого каркаса можно использовать [генератор модулей](guides/new_module.md); ниже — ручная сборка и обязательные артефакты.
...
- Возвращайте `JsonResponse` или [`FileResponse`](reference/back-end/classes/FileResponse.md) (скачивание файлов, DevCraft ≥ 200.4.0)
- Загрузка на сервер: [`UploadedFile`](reference/back-end/classes/UploadedFile.md) + [`postMultipart`](reference/front-end/templates.md#multipart-загрузка-файлов)
...
- [Создание CRUD-страницы](guides/crud_pages.md) — сквозной сценарий: Entity → Repository → список с фильтром → форма → Ajax → манифест → Twig
- [Примеры Form](guides/form_examples.md) — layout stack/tabs/accordion, все типы полей, filter schema
- [Генератор модулей](guides/new_module.md)
- [Composer-пакеты модулей](reference/back-end/composer_packages.md)
- [Манифест модуля](reference/back-end/manifest.md)
- [Точки входа](reference/back-end/entry_points.md)
- [ИИ-инструменты](guides/ai_tools.md)
