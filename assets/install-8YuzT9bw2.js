import{r as e}from"./rolldown-runtime-QTnfLwEv.js";import{t}from"./jsx-runtime-By8HlURe.js";var n=e(t()),r={title:`Установка`,description:`Как поставить DLE API после DevCraft Admin: Composer в api/, таблицы, профиль, nginx.`,version:`200.1.1`},i=new Date(1789411935e3),a=`
...
<Callout type="warn" title="Сначала DevCraft Admin">
  Установите и включите [DevCraft Admin](/dev/dle/devcraft_admin/200.4.1/install), затем DLE API. Без оболочки модуль в админке не поднимется как задумано, а таблицы \`api_*\` не создадутся через Cycle.
</Callout>
...
* \`engine/inc/dleapi.php\` — тонкий вход в админку DLE;
* \`devcraft/src/modules/DleApi/\` — модуль DevCraft (ключи, OAuth, настройки);
* \`api/\` — runtime \`/api/v2\` (Slim, OAuth2, OpenAPI);
* локаль \`devcraft/locales/ru_RU/dleapi.xliff\`.

In-process SDK (\`DcApi\`) — в DevCraft Admin: \`devcraft/src/sdk/dle/\`, не в этом ZIP.

### Composer в \`api/\` [#composer-в-api]

Зависимости HTTP-слоя лежат в &#x2A;*\`api/\`** (отдельно от \`devcraft/\`):

\`\`\`bash
cd api && composer install
\`\`\`

Общий Composer для оболочки: [Установка плагинов](/instructions/install_instructions) и [Composer](/instructions/composer).

### Таблицы API [#таблицы-api]

Откройте модуль **DLE API** в DevCraft. Таблицы создаёт **Cycle ORM** Admin (не DDL в \`install.xml\`):

* \`{prefix}_api_keys\`, \`{prefix}_api_scope\`
* \`{prefix}_api_access_levels\`, \`{prefix}_api_access_level_scopes\`, \`{prefix}_api_access_level_groups\`
* \`{prefix}_api_key_requests\`
* \`{prefix}_api_oauth_*\`

Миграции пишутся в \`devcraft/src/database/migrations/\` при пересборке схемы ORM (см. [DatabaseGateway](/dev/dle/devcraft_admin/200.4.1/reference/back-end/classes/DatabaseGateway#method_generateOrm) в Admin ≥ **200.4.1**).

Если видите \`SQLSTATE[42S02]\` (таблица не найдена):

1. Убедитесь, что установлен DevCraft Admin ≥ **200.4.1**.
2. Удалите кэш \`devcraft/cache/cycle_orm_schema.ser\`.
3. Снова откройте раздел DLE API в админке — migrator создаст недостающие таблицы.

### Дальше по настройке [#дальше-по-настройке]

1. Создайте API-ключ и при необходимости [OAuth-клиент](guides/oauth-client).
...
`,o={contents:[{heading:void 0,content:"Как поставить ZIP через менеджер плагинов DLE, скрипт архива или копирование `upload/` — в общей инструкции: **Установка плагинов**."},{heading:void 0,content:`Ниже — только то, что **специфично для DLE API**.`},{heading:`требования-модуля`,content:`Компонент`},{heading:`требования-модуля`,content:`Версия`},{heading:`требования-модуля`,content:`DataLife Engine`},{heading:`требования-модуля`,content:`≥ 20.0`},{heading:`требования-модуля`,content:`PHP`},{heading:`требования-модуля`,content:`≥ 8.3`},{heading:`требования-модуля`,content:`DevCraft Admin`},{heading:`требования-модуля`,content:`≥ 200.4.1`},{heading:`требования-модуля`,content:`MySQL / MariaDB`},{heading:`требования-модуля`,content:`InnoDB (внешние ключи)`},{heading:`требования-модуля`,content:"Установите и включите DevCraft Admin, затем DLE API. Без оболочки модуль в админке не поднимется как задумано, а таблицы `api_*` не создадутся через Cycle."},{heading:`после-установки-плагина`,content:`На сайте должны появиться:`},{heading:`после-установки-плагина`,content:"`engine/inc/dleapi.php` — тонкий вход в админку DLE;"},{heading:`после-установки-плагина`,content:"`devcraft/src/modules/DleApi/` — модуль DevCraft (ключи, OAuth, настройки);"},{heading:`после-установки-плагина`,content:"`api/` — runtime `/api/v2` (Slim, OAuth2, OpenAPI);"},{heading:`после-установки-плагина`,content:"локаль `devcraft/locales/ru_RU/dleapi.xliff`."},{heading:`после-установки-плагина`,content:"In-process SDK (`DcApi`) — в DevCraft Admin: `devcraft/src/sdk/dle/`, не в этом ZIP."},{heading:`composer-в-api`,content:"Зависимости HTTP-слоя лежат в &#x2A;*`api/`** (отдельно от `devcraft/`):"},{heading:`composer-в-api`,content:`Общий Composer для оболочки: Установка плагинов и Composer.`},{heading:`таблицы-api`,content:"Откройте модуль **DLE API** в DevCraft. Таблицы создаёт **Cycle ORM** Admin (не DDL в `install.xml`):"},{heading:`таблицы-api`,content:"`{prefix}_api_keys`, `{prefix}_api_scope`"},{heading:`таблицы-api`,content:"`{prefix}_api_access_levels`, `{prefix}_api_access_level_scopes`, `{prefix}_api_access_level_groups`"},{heading:`таблицы-api`,content:"`{prefix}_api_key_requests`"},{heading:`таблицы-api`,content:"`{prefix}_api_oauth_*`"},{heading:`таблицы-api`,content:"Миграции пишутся в `devcraft/src/database/migrations/` при пересборке схемы ORM (см. DatabaseGateway в Admin ≥ **200.4.1**)."},{heading:`таблицы-api`,content:"Если видите `SQLSTATE[42S02]` (таблица не найдена):"},{heading:`таблицы-api`,content:`Убедитесь, что установлен DevCraft Admin ≥ **200.4.1**.`},{heading:`таблицы-api`,content:"Удалите кэш `devcraft/cache/cycle_orm_schema.ser`."},{heading:`таблицы-api`,content:`Снова откройте раздел DLE API в админке — migrator создаст недостающие таблицы.`},{heading:`дальше-по-настройке`,content:`Создайте API-ключ и при необходимости OAuth-клиент.`},{heading:`дальше-по-настройке`,content:"Проверьте `.env` (`DEMO_MODE=false`, если нужны токены)."},{heading:`дальше-по-настройке`,content:`Авторизация → эндпоинты.`},{heading:`профиль-на-сайте-опционально`,content:`Кнопка «Запросить ключ» сама не появляется. Нужны include в скине и флаги в Настройках. Теги шаблона: Шаблоны.`},{heading:`профиль-на-сайте-опционально`,content:`Включите **Показывать блок** / **Разрешить генерировать** и задайте уровень или синхронизацию групп.`},{heading:`профиль-на-сайте-опционально`,content:"В `userinfo.tpl` (без `[own-profile]`):"},{heading:`профиль-на-сайте-опционально`,content:"В `main.tpl` перед `</body>`:"},{heading:`профиль-на-сайте-опционально`,content:"Очистите кеш шаблонов. Переопределить вид: скопируйте `templates/Default/devcraft/dleapi/` в свою тему."},{heading:`профиль-на-сайте-опционально`,content:`Без шага с JS в блоке будет «DevCraftPublic недоступен». Гость чужой ключ не видит; админ на чужом профиле может одобрять заявки.`},{heading:`редирект-api--apiv2`,content:"**Apache:** правила уже в `api/.htaccess`."},{heading:`редирект-api--apiv2`,content:"**nginx** (фрагмент `server`):"},{heading:`см-также`,content:`Установка плагинов`},{heading:`см-также`,content:`Начало работы`},{heading:`см-также`,content:`Инструкция .env`}],headings:[{id:`требования-модуля`,content:`Требования модуля`},{id:`после-установки-плагина`,content:`После установки плагина`},{id:`composer-в-api`,content:"Composer в `api/`"},{id:`таблицы-api`,content:`Таблицы API`},{id:`дальше-по-настройке`,content:`Дальше по настройке`},{id:`профиль-на-сайте-опционально`,content:`Профиль на сайте (опционально)`},{id:`редирект-api--apiv2`,content:"Редирект `/api` → `/api/v2`"},{id:`см-также`,content:`См. также`}]},s=[{depth:2,url:`#требования-модуля`,title:(0,n.jsx)(n.Fragment,{children:`Требования модуля`})},{depth:2,url:`#после-установки-плагина`,title:(0,n.jsx)(n.Fragment,{children:`После установки плагина`})},{depth:3,url:`#composer-в-api`,title:(0,n.jsxs)(n.Fragment,{children:[`Composer в `,(0,n.jsx)(`code`,{children:`api/`})]})},{depth:3,url:`#таблицы-api`,title:(0,n.jsx)(n.Fragment,{children:`Таблицы API`})},{depth:3,url:`#дальше-по-настройке`,title:(0,n.jsx)(n.Fragment,{children:`Дальше по настройке`})},{depth:2,url:`#профиль-на-сайте-опционально`,title:(0,n.jsx)(n.Fragment,{children:`Профиль на сайте (опционально)`})},{depth:2,url:`#редирект-api--apiv2`,title:(0,n.jsxs)(n.Fragment,{children:[`Редирект `,(0,n.jsx)(`code`,{children:`/api`}),` → `,(0,n.jsx)(`code`,{children:`/api/v2`})]})},{depth:2,url:`#см-также`,title:(0,n.jsx)(n.Fragment,{children:`См. также`})}];function c(e){let t={a:`a`,code:`code`,h2:`h2`,h3:`h3`,li:`li`,ol:`ol`,p:`p`,pre:`pre`,span:`span`,strong:`strong`,table:`table`,tbody:`tbody`,td:`td`,th:`th`,thead:`thead`,tr:`tr`,ul:`ul`,...e.components},{Callout:r}=t;return r||u(`Callout`,!0),(0,n.jsxs)(n.Fragment,{children:[(0,n.jsxs)(t.p,{children:[`Как поставить ZIP через менеджер плагинов DLE, скрипт архива или копирование `,(0,n.jsx)(t.code,{children:`upload/`}),` — в общей инструкции: `,(0,n.jsx)(t.strong,{children:(0,n.jsx)(t.a,{href:`/instructions/install_instructions`,children:`Установка плагинов`})}),`.`]}),`
...
`,(0,n.jsxs)(t.table,{children:[(0,n.jsx)(t.thead,{children:(0,n.jsxs)(t.tr,{children:[(0,n.jsx)(t.th,{children:`Компонент`}),(0,n.jsx)(t.th,{children:`Версия`})]})}),(0,n.jsxs)(t.tbody,{children:[(0,n.jsxs)(t.tr,{children:[(0,n.jsx)(t.td,{children:`DataLife Engine`}),(0,n.jsx)(t.td,{children:`≥ 20.0`})]}),(0,n.jsxs)(t.tr,{children:[(0,n.jsx)(t.td,{children:`PHP`}),(0,n.jsx)(t.td,{children:`≥ 8.3`})]}),(0,n.jsxs)(t.tr,{children:[(0,n.jsx)(t.td,{children:`DevCraft Admin`}),(0,n.jsx)(t.td,{children:`≥ 200.4.1`})]}),(0,n.jsxs)(t.tr,{children:[(0,n.jsx)(t.td,{children:`MySQL / MariaDB`}),(0,n.jsx)(t.td,{children:`InnoDB (внешние ключи)`})]})]})]}),`
`,(0,n.jsx)(r,{type:`warn`,title:`Сначала DevCraft Admin`,children:(0,n.jsxs)(t.p,{children:[`Установите и включите `,(0,n.jsx)(t.a,{href:`/dev/dle/devcraft_admin/200.4.1/install`,children:`DevCraft Admin`}),`, затем DLE API. Без оболочки модуль в админке не поднимется как задумано, а таблицы `,(0,n.jsx)(t.code,{children:`api_*`}),` не создадутся через Cycle.`]})}),`
`,(0,n.jsx)(t.h2,{id:`после-установки-плагина`,children:`После установки плагина`}),`
`,(0,n.jsx)(t.p,{children:`На сайте должны появиться:`}),`
`,(0,n.jsxs)(t.ul,{children:[`
`,(0,n.jsxs)(t.li,{children:[(0,n.jsx)(t.code,{children:`engine/inc/dleapi.php`}),` — тонкий вход в админку DLE;`]}),`
`,(0,n.jsxs)(t.li,{children:[(0,n.jsx)(t.code,{children:`devcraft/src/modules/DleApi/`}),` — модуль DevCraft (ключи, OAuth, настройки);`]}),`
`,(0,n.jsxs)(t.li,{children:[(0,n.jsx)(t.code,{children:`api/`}),` — runtime `,(0,n.jsx)(t.code,{children:`/api/v2`}),` (Slim, OAuth2, OpenAPI);`]}),`
`,(0,n.jsxs)(t.li,{children:[`локаль `,(0,n.jsx)(t.code,{children:`devcraft/locales/ru_RU/dleapi.xliff`}),`.`]}),`
`]}),`
`,(0,n.jsxs)(t.p,{children:[`In-process SDK (`,(0,n.jsx)(t.code,{children:`DcApi`}),`) — в DevCraft Admin: `,(0,n.jsx)(t.code,{children:`devcraft/src/sdk/dle/`}),`, не в этом ZIP.`]}),`
`,(0,n.jsxs)(t.h3,{id:`composer-в-api`,children:[`Composer в `,(0,n.jsx)(t.code,{children:`api/`})]}),`
`,(0,n.jsxs)(t.p,{children:[`Зависимости HTTP-слоя лежат в `,(0,n.jsx)(t.strong,{children:(0,n.jsx)(t.code,{children:`api/`})}),` (отдельно от `,(0,n.jsx)(t.code,{children:`devcraft/`}),`):`]}),`
...
`,(0,n.jsxs)(t.p,{children:[`Общий Composer для оболочки: `,(0,n.jsx)(t.a,{href:`/instructions/install_instructions`,children:`Установка плагинов`}),` и `,(0,n.jsx)(t.a,{href:`/instructions/composer`,children:`Composer`}),`.`]}),`
`,(0,n.jsx)(t.h3,{id:`таблицы-api`,children:`Таблицы API`}),`
`,(0,n.jsxs)(t.p,{children:[`Откройте модуль `,(0,n.jsx)(t.strong,{children:`DLE API`}),` в DevCraft. Таблицы создаёт `,(0,n.jsx)(t.strong,{children:`Cycle ORM`}),` Admin (не DDL в `,(0,n.jsx)(t.code,{children:`install.xml`}),`):`]}),`
`,(0,n.jsxs)(t.ul,{children:[`
`,(0,n.jsxs)(t.li,{children:[(0,n.jsx)(t.code,{children:`{prefix}_api_keys`}),`, `,(0,n.jsx)(t.code,{children:`{prefix}_api_scope`})]}),`
`,(0,n.jsxs)(t.li,{children:[(0,n.jsx)(t.code,{children:`{prefix}_api_access_levels`}),`, `,(0,n.jsx)(t.code,{children:`{prefix}_api_access_level_scopes`}),`, `,(0,n.jsx)(t.code,{children:`{prefix}_api_access_level_groups`})]}),`
`,(0,n.jsx)(t.li,{children:(0,n.jsx)(t.code,{children:`{prefix}_api_key_requests`})}),`
`,(0,n.jsx)(t.li,{children:(0,n.jsx)(t.code,{children:`{prefix}_api_oauth_*`})}),`
`]}),`
`,(0,n.jsxs)(t.p,{children:[`Миграции пишутся в `,(0,n.jsx)(t.code,{children:`devcraft/src/database/migrations/`}),` при пересборке схемы ORM (см. `,(0,n.jsx)(t.a,{href:`/dev/dle/devcraft_admin/200.4.1/reference/back-end/classes/DatabaseGateway#method_generateOrm`,children:`DatabaseGateway`}),` в Admin ≥ `,(0,n.jsx)(t.strong,{children:`200.4.1`}),`).`]}),`
`,(0,n.jsxs)(t.p,{children:[`Если видите `,(0,n.jsx)(t.code,{children:`SQLSTATE[42S02]`}),` (таблица не найдена):`]}),`
`,(0,n.jsxs)(t.ol,{children:[`
`,(0,n.jsxs)(t.li,{children:[`Убедитесь, что установлен DevCraft Admin ≥ `,(0,n.jsx)(t.strong,{children:`200.4.1`}),`.`]}),`
`,(0,n.jsxs)(t.li,{children:[`Удалите кэш `,(0,n.jsx)(t.code,{children:`devcraft/cache/cycle_orm_schema.ser`}),`.`]}),`
`,(0,n.jsx)(t.li,{children:`Снова откройте раздел DLE API в админке — migrator создаст недостающие таблицы.`}),`
`]}),`
`,(0,n.jsx)(t.h3,{id:`дальше-по-настройке`,children:`Дальше по настройке`}),`
`,(0,n.jsxs)(t.ol,{children:[`
`,(0,n.jsxs)(t.li,{children:[`Создайте API-ключ и при необходимости `,(0,n.jsx)(t.a,{href:`guides/oauth-client`,children:`OAuth-клиент`}),`.`]}),`
