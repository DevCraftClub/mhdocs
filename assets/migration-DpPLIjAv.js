import{r as e}from"./rolldown-runtime-QTnfLwEv.js";import{t}from"./jsx-runtime-By8HlURe.js";var n=e(t()),r={title:`Миграция на DevCraft Admin`,description:`Перенос с MH Admin (Legacy) на DevCraft Admin для DLE 20.0.`},i=new Date(1787217967e3),a=`
...
Дополнительно меняется модель повторного использования кода: общие DTO, reflection/fluent-абстракции и runtime-хелперы больше не обязаны жить внутри самой админ-оболочки. Для этого постепенно используется внешний пакет [\`devcraftclub/dev-tools\`](/dev/dev-tools/1.0.1/getting_started). См. [Shared-пакеты и абстракции](reference/back-end/shared_packages).
...
Если при переносе вы находите старые универсальные helper/DTO-классы, сначала проверьте, не должен ли этот слой переехать в \`devcraftclub/dev-tools\`, а не остаться внутри конкретного admin-модуля.
...
`,(0,n.jsxs)(t.p,{children:[`Дополнительно меняется модель повторного использования кода: общие DTO, reflection/fluent-абстракции и runtime-хелперы больше не обязаны жить внутри самой админ-оболочки. Для этого постепенно используется внешний пакет `,(0,n.jsx)(t.a,{href:`/dev/dev-tools/1.0.1/getting_started`,children:(0,n.jsx)(t.code,{children:`devcraftclub/dev-tools`})}),`. См. `,(0,n.jsx)(t.a,{href:`reference/back-end/shared_packages`,children:`Shared-пакеты и абстракции`}),`.`]}),`
...
`,(0,n.jsxs)(t.p,{children:[`Если при переносе вы находите старые универсальные helper/DTO-классы, сначала проверьте, не должен ли этот слой переехать в `,(0,n.jsx)(t.code,{children:`devcraftclub/dev-tools`}),`, а не остаться внутри конкретного admin-модуля.`]}),`
