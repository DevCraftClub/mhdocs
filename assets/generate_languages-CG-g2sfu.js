import{r as e}from"./rolldown-runtime-QTnfLwEv.js";import{t}from"./jsx-runtime-By8HlURe.js";var n=e(t()),r={title:`Генератор языковых файлов`,description:`Локализация модулей DevCraft: XLIFF и Translation.`},i=new Date(1787141927e3),a=`

Переводимые строки в коде оборачиваются в \`__()\` / \`translate()\` (см. \`devcraft/src/bootstrap/functions.php\`). Файлы локалей хранятся вне \`src/\` — по умолчанию в \`devcraft/locales/{locale}/\` (настраивается полем **Путь до языков** на странице [Настройка](manage)).

Блок путей в глобальных настройках DevCraft Admin задаёт каталог локалей. Для graphify здесь важнее явно связать страницу с настройкой \`locales_path\`, чем с отдельным изображением.
