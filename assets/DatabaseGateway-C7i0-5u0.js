...
При **пересборке схемы** (нет файла кэша \`cycle_orm_schema.ser\` или изменилась подпись Models модулей) всегда включается **GenerateMigrations**: Cycle пишет diff в \`devcraft/src/database/migrations/\` и migrator применяет недостающие \`CREATE\`/\`ALTER\`. Так появляются таблицы сателлитов (\`api_*\`, faker, …) **после** того, как таблицы ядра Admin уже существуют.

Раньше GenerateMigrations срабатывал только при отсутствии ядра (\`devcraft_migrations\` / \`logs\` / \`composer_data\`) или константы \`DEVCRAFT_GENERATE_MIGRATIONS\` — из‑за этого на «чистом» сайте после первого захода Admin новые модули давали \`SQLSTATE[42S02]\`.

Существующие таблицы пропускаются через \`skipCreateMigrationsForExistingTables\`.
...
`,(0,n.jsxs)(t.p,{children:[`При `,(0,n.jsx)(t.strong,{children:`пересборке схемы`}),` (нет файла кэша `,(0,n.jsx)(t.code,{children:`cycle_orm_schema.ser`}),` или изменилась подпись Models модулей) всегда включается `,(0,n.jsx)(t.strong,{children:`GenerateMigrations`}),`: Cycle пишет diff в `,(0,n.jsx)(t.code,{children:`devcraft/src/database/migrations/`}),` и migrator применяет недостающие `,(0,n.jsx)(t.code,{children:`CREATE`}),`/`,(0,n.jsx)(t.code,{children:`ALTER`}),`. Так появляются таблицы сателлитов (`,(0,n.jsx)(t.code,{children:`api_*`}),`, faker, …) `,(0,n.jsx)(t.strong,{children:`после`}),` того, как таблицы ядра Admin уже существуют.`]}),`
`,(0,n.jsxs)(t.p,{children:[`Раньше GenerateMigrations срабатывал только при отсутствии ядра (`,(0,n.jsx)(t.code,{children:`devcraft_migrations`}),` / `,(0,n.jsx)(t.code,{children:`logs`}),` / `,(0,n.jsx)(t.code,{children:`composer_data`}),`) или константы `,(0,n.jsx)(t.code,{children:`DEVCRAFT_GENERATE_MIGRATIONS`}),` — из‑за этого на «чистом» сайте после первого захода Admin новые модули давали `,(0,n.jsx)(t.code,{children:`SQLSTATE[42S02]`}),`.`]}),`
`,(0,n.jsxs)(t.p,{children:[`Существующие таблицы пропускаются через `,(0,n.jsx)(t.code,{children:`skipCreateMigrationsForExistingTables`}),`.`]}),`
`,(0,n.jsxs)(t.p,{children:[(0,n.jsx)(t.strong,{children:`С версии:`}),` 171.3.0`]}),`
